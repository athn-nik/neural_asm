...
def merged_embeddings(a, dim_a, b, dim_b):
    word2idx_a, idx2word_a, embeddings_a = load_word_vectors(a, dim_a)
    word2idx_b, idx2word_b, embeddings_b = load_word_vectors(b, dim_b)

    word2idx = word2idx_a
    idx2word = idx2word_a

    for word, idx in word2idx_b.items():
        if word not in word2idx_a:
            new_idx = len(word2idx_a) + 1
            word2idx[word] = new_idx
            idx2word[new_idx] = word

    embeddings = numpy.zeros((len(word2idx) + 1, dim_a + dim_b))

    for word, idx in word2idx.items():
        if word in word2idx_a and word in word2idx_b:
            veca = embeddings_a[word2idx_a[word]]
            vecb = embeddings_b[word2idx_b[word]]

        elif not word in word2idx_a and word in word2idx_b:
            veca = numpy.zeros(dim_a)
            vecb = embeddings_b[word2idx_b[word]]

        elif word in word2idx_a and not word in word2idx_b:
            veca = embeddings_a[word2idx_a[word]]
            vecb = numpy.zeros(dim_b)
        else:
            raise ValueError("wtf")

        embeddings[idx] = numpy.concatenate((veca, vecb), axis=0)

    return word2idx, idx2word, embeddings.astype("float32")
